...
namespace exolix {
    std::tuple<ColorRgb, long long>
    AlgorithmSearch::findClosestRgb(const std::vector<ColorRgb> &list, const exolix::ColorRgb &rgb) {
        ...
        for (long long i = 0; i < list.size(); i++) {
            ...
            long long currentDistance = std::abs(currentRgb.red - rgb.red) + std::abs(currentRgb.green - rgb.green) +
                                        std::abs(currentRgb.blue - rgb.blue);
