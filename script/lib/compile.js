...
export async function cmakeInit() {
    const buildExists = fs.existsSync(path.join(projectRoot, "build"))
    ...
    if (!buildExists) {
        ...
        const returnCode = await commandRunnerAsync("build", "cmake ..", "CMake Init");
...
export async function cmakeBuild(release) {
    ...
    const returnCode = await commandRunnerAsync("build", "cmake --build . --config " + (release ? "Release" : "Debug"), "CMake Build");
